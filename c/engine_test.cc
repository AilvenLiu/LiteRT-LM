...
TEST(EngineCTest, Benchmark) {
  ...
  EngineSettingsPtr settings(
      ...
      &litert_lm_engine_settings_delete);
  ASSERT_NE(settings, nullptr);
  litert_lm_engine_settings_set_max_num_tokens(settings.get(), 288);
  litert_lm_engine_settings_set_benchmark_params(settings.get(), 128, 128);
  ...
  EXPECT_GT(litert_lm_benchmark_info_get_total_init_time(benchmark_info.get()),
            0.0);
  EXPECT_GT(
