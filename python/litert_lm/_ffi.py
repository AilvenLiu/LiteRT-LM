...
def _get_lib():
  ...
  lib_name = "liblitert-lm.so"

  # Try loading using importlib.resources for .par support
  try:
  ...
  # Fallback to runfiles directory (__file__) for Bazel
  try:
  ...
  try:
    ...
    raise RuntimeError(
        f"Could not load {lib_name}. Ensure it is built and in LD_LIBRARY_PATH."
    ) from e
